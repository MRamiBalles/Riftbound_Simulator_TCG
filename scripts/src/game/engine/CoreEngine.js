...
const RuntimeCard_1 = require("./RuntimeCard");
...
class CoreEngine {
     ...
     */
    initGame(playerDeck, opponentDeck, seed) {
    ...
    createInitialState(seed) {
        return {
            ...
            combat: null,
            stack: [],
            seed: seed !== null && seed !== void 0 ? seed : Math.floor(Math.random() * 1000000), // Default random seed
            actionHistory: []
        };
    }
    createEmptyPlayerState(id) {
        return {
            id,
            health: 20,
            maxHealth: 20,
    ...
    initializePlayer(id, deck) {
        // Deterministic shuffle
        const shuffled = this.deterministicShuffle([...deck]);
        // Use seeded ID generation
        const runtimeDeck = shuffled.map(c => (0, RuntimeCard_1.createRuntimeCard)(c, id, this.generateUUID()));
    ...
    applyAction(action) {
        ...
        if (!this.state.actionHistory)
            this.state.actionHistory = [];
        this.state.actionHistory.push(Object.assign({}, action));
    ...
    handlePlayCard(action) {
        ...
        if (card.type === 'Unit' || card.type === 'Champion' || card.type === 'Legend') {
            card.summoningSickness = !((_a = card.keywords) === null || _a === void 0 ? void 0 : _a.includes('Rush'));
    ...
    handlePass(playerId) {
        if (this.state.priority !== playerId)
            return;
    ...
    applyTargetEffect(sourceId, targetId, sourceCard) {
        // Legacy fallback for cards without declarative effects
        if ((sourceCard === null || sourceCard === void 0 ? void 0 : sourceCard.effects) && sourceCard.effects.length > 0) {
            const context = {
            ...
            const { state: newState, result } = effects_1.EffectResolver.resolve(this.state, sourceCard, 'ON_CAST', context);
    ...
    resolveSpell(card, targetId) {
        ...
        if (card.effects && card.effects.length > 0) {
            ...
            const { state: newState, result } = effects_1.EffectResolver.resolve(this.state, card, 'ON_CAST', context);
            this.state = newState;
            this.state.log.push(...result.log);
            this.cleanupDeadUnits(result.deadUnits);
            return;
        ...
        if (targetId) {
            if (targetId === 'opponent' || targetId === 'player') {
                this.state.players[targetId].health -= 2;
     ...
     */
    triggerEffects(card, trigger, selectedTargetId) {
        if (!card.effects || card.effects.length === 0)
            return;
        const context = {
            seed: this.state.seed,
        ...
        const { state: newState, result } = effects_1.EffectResolver.resolve(this.state, card, trigger, context);
        this.state = newState;
        this.state.log.push(...result.log);
        this.cleanupDeadUnits(result.deadUnits);
    }
    handleDeclareAttackers(action) {
    ...
    handleDeclareBlockers(action) {
        if (!this.state.combat || this.state.combat.step !== 'declare_blockers')
            return;
        ...
        if (action.blockers) {
            Object.entries(action.blockers).forEach(([blockerInstanceId, attackerInstanceId]) => {
                ...
                if (blocker && attacker) {
                    ...
                    const isAttackerElusive = attacker.keywords.includes('Elusive');
                    const isBlockerElusive = blocker.keywords.includes('Elusive');
    ...
    handleResolveCombat() {
        if (!this.state.combat)
            return;
        const result = CombatResolver_1.CombatResolver.resolveCombat(this.state, this.state.combat);
        // 1. Apply Damage Events
        result.damageEvents.forEach(evt => {
            if (evt.targetId === 'player' || evt.targetId === 'opponent') {
                this.state.players[evt.targetId].health -= evt.amount;
            }
            else {
                ['player', 'opponent'].forEach(pid => {
                    const p = this.state.players[pid];
        ...
        // Check Win
        if (this.state.players.player.health <= 0)
            this.state.winner = 'opponent';
        if (this.state.players.opponent.health <= 0)
    ...
    exportReplay(p1Name = 'Player 1', p2Name = 'Player 2') {
        var _a, _b;
        return {
            metadata: {
            ...
            initialState: {
                p1Deck: ((_a = this.initialState) === null || _a === void 0 ? void 0 : _a.p1Deck.map(c => c.id)) || [],
                p2Deck: ((_b = this.initialState) === null || _b === void 0 ? void 0 : _b.p2Deck.map(c => c.id)) || [],
