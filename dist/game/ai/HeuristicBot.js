...
class HeuristicBot {
    ...
    difficulty;
    fastMode;
    constructor(id = 'opponent', name = 'Noxian General', difficulty = 'Medium', fastMode = false) {
        this.id = id;
        this.name = name;
     ...
     */
    checkLethalOpportunity(gameState) {
        const myUnits = gameState.players[this.id].field;
        const opponentId = this.id === 'player' ? 'opponent' : 'player';
        const opponent = gameState.players[opponentId];
        const potentialAttackers = myUnits.filter((u) => !u.hasAttacked && !u.summoningSickness && !u.isStunned);
        const totalPower = potentialAttackers.reduce((sum, u) => sum + u.currentAttack, 0);
    ...
    decideAttack(gameState) {
        ...
        const attackers = potentialAttackers.filter((u) => {
            ...
            const strongestOpponent = opponentField.length > 0 ? Math.max(...opponentField.map((o) => o.currentAttack)) : 0;
        ...
        if (attackers.length > 0) {
            return {
                type: 'DECLARE_ATTACKERS',
                playerId: this.id,
                attackers: attackers.map((u) => u.instanceId)
            };
        }
        return null;
    }
    decideBlockers(gameState) {
        const myUnits = gameState.players[this.id].field;
        const attackingIds = Object.keys(gameState.combat?.attackers || {});
        const opponentId = this.id === 'player' ? 'opponent' : 'player';
        const opponentField = gameState.players[opponentId].field;
        const blockers = {};
        const availableBlockers = myUnits.filter((u) => !u.isStunned);
        attackingIds.forEach(attackerId => {
            const attacker = opponentField.find((u) => u.instanceId === attackerId);
            if (!attacker)
                return;
            // Try to find a profitable or safe block
            const bestBlocker = availableBlockers.find((b) => {
            ...
            if (bestBlocker) {
                ...
                if (index > -1)
                    availableBlockers.splice(index, 1);
            }
        });
