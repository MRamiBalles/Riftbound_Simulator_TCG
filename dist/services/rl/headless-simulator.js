...
class HeadlessSimulator {
    ...
    static async runGame(difficulty = 'Medium') {
        ...
        const bots = {
            'player': new HeuristicBot_1.HeuristicBot('player', 'Trainer P1', difficulty, true),
            'opponent': new HeuristicBot_1.HeuristicBot('opponent', 'Trainer P2', difficulty, true)
        };
        const trajectory = [];
        ...
        while (!engine.getState().winner && turn < turnLimit) {
            ...
            if (action) {
                ...
                trajectory.push({
                    state: encoding_service_1.EncodingService.encode(state, activeId),
                    actionIdx,
                    playerId: activeId
                });
     ...
     */
    static inferActionIndex(action, state) {
        if (action.type === 'PASS' || action.type === 'END_TURN')
            return 0;
...
exports.HeadlessSimulator = HeadlessSimulator;
